...
import { deleteTodoList, editTodoList, deleteTodo } from '../../actions/index';
...
class Todo extends React.Component {
  ...
  removeTodoList = id => {
    ...
    this.props.todos.filter(todo => todo.todoListId === id).map(todo => this.props.deleteTodo(todo.id));
  };
        ...
        {(this.props.lists.length) ? (
          <List divided verticalAlign='middle'>
            {this.props.lists.map((list, index) => {
              return (
                <List.Item key={list.id} style={(index % 2 === 0) ? {
...
export default connect(mapStateToProps, { deleteTodoList, editTodoList, deleteTodo })(Todo);